...
try
    ...
    par.runID = 'INITIALS_DelaysDotsStair';
    ...
    dlg_title = 'Dot Motion Delays Task';
    while 1
        prompt = {'Enter SUBJECT/Task','EEG? (1=yes, 0=no)', 'Number of Trials (multiples of 6)', 'Block', 'Booth(1/2/3)'};
        def = {par.runID,num2str(1), num2str(54), num2str(block), '1'};
        answer = inputdlg(prompt,dlg_title,1,def);
        par.runID = answer{1};
    ...
    par.cohLevels = 100; % 0 10 20 40 coherence levels - these will be randomly interleaved - separate by spaces or commas [0 10 20 40] [0 80]
    ...
    CohLevs = [75 50 40 30 25 20 15 13 10 8 7 6 5 4 3 2 1];
    stair=1;
    ...
    temp = repmat(1:6,[1,par.numtrials/6]);
   ...
   %% Record some details for later use   
        clicked=0;
        trialCond{block}(n) = temp(n); % 1 = 800ms delay left, 2=800ms delay right, 3=1200ms delay left, 4=1200ms right, 5 = 1600ms delay left, 6=1600ms delay right
...
if n>2 % start the staircase from the 3rd trial
if Acc{block}(end) ==0; % if the last trial for this delay period produced an error then make it easier 
    stair=stair-1;  
    move{block}(n) = -1; 
elseif Acc{block}(end)==1 && Acc{block}(end-1) ==1 && ~move{block}(end-1) %% If the last 2 trials for this delay have been correct you didn't move on the last correct trial
    stair =stair+1; 
   move{block}(n) =1; 
end
end 
...
if stair<1
    stair=1; % Never let stair be lower than 1
end 

par.cohLevels = CohLevs(stair); 
par.TrialCoh{block}(n) = par.cohLevels; 
        
        %% MAKE THE DOTS FOR THIS TRIAL
        clear dots
        coh{trialCond{block}(n)}(trialLR{block}(n), DelayF:end)=par.cohLevels; 
        numFr = size(coh{trialCond{block}(n)},2);
        ...
        for i=1:numFr
            ...
            if i==1
                nPTBevent = nPTBevent+1;    % increment the number of events
                PTBevent(block,nPTBevent) = par.CD_DOTS_ON;    % record the event that just happened (using the arbitrarily defined codes above)
            ...
            elseif i>DelayF-1
                ...
                if i == DelayF
                    targtime=GetSecs;
                    nPTBevent = nPTBevent+1;    % increment the number of events
                    PTBevent(block,nPTBevent) = par.CD_COHMOTION_ON(trialCond{block}(n));    % record the event that just happened (using the arbitrarily defined codes above)
                    [VBLTimestamp PTBeventT(block,nPTBevent)] = Screen('Flip', window);
                ...
                if any(buttons) && clicked==0;
                    if buttons(1), resp.LR{block}(n) = 1;
                        ...
                        nPTBevent = nPTBevent+1;    % increment the number of events
                        PTBevent(block,nPTBevent) = par.CD_BUTTONS(1);    % record the event that just happened (using the arbitrarily defined codes above)
                        PTBeventT(block,nPTBevent) = GetSecs;
                        clicked = 1;
                    ...
                    elseif buttons(end), resp.LR{block}(n) = 2;
                        ...
                        nPTBevent = nPTBevent+1;    % increment the number of events
                        PTBevent(block,nPTBevent) = par.CD_BUTTONS(2);    % record the event that just happened (using the arbitrarily defined codes above)
                        PTBeventT(block,nPTBevent) = GetSecs;
                        clicked = 1;
                        resp.time{block}(n)= GetSecs-targtime;
                    end
        ...
        %% Trial Over
        nPTBevent = nPTBevent+1;    % increment the number of events
        PTBevent(block,nPTBevent) = par.CD_COHMOTION_OFF;    % record the event that just happened (using the arbitrarily defined codes above)
        [VBLTimestamp PTBeventT(block,nPTBevent)] = Screen('Flip', window);
        ...
        %% Give threshold feedback
        
        if n==par.numtrials;
            
            par.MeanThreshoold(block) = nanmean(par.TrialCoh{block}(end-10:end)); 
            par.ModeThreshoold(block) = mode(par.TrialCoh{block}(end-10:end)); 

            Screen('DrawText', window, 'Discrimination Threshold', 0.35*scres(1), 0.25*scres(2), 255);
            Screen('DrawText', window, 'Median = num2str(par.MedianThreshold)', 0.4*scres(1), 0.45*scres(2), 255);
            Screen('DrawText', window, 'Median = num2str(par.ModeThreshold)', 0.4*scres(1), 0.55*scres(2), 255);
            Screen('Flip', window);
            ...
            save([par.runID 'Threshold'],'PTBeventT','nPTBevent','PTBevent','resp', 'trialLR','par', 'savealldots', 'block', 'Delay')  % save([par.runID],'ITIstartT','TargOnT','RespT','RespLR','trialITI','trialLR','par','resp')

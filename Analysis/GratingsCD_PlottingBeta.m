...
ylims=[-.5 .5];
sp(6)=subplot(2,7,6); topoplot(abs(nanmean(nanmean(nanmean(nanmean(TopoRT(:,1,:,:,subs)-TopoRT(:,2,:,:,subs),2),3),4),5)), chanlocs,'plotchans',[1:128],'maplimits', ylims, ...
...
for z=1:3
    %pl2(z)=plot(nanmean(Plot_RTBins(:,z,subs),3),abs(nanmean(BetaContraBins(:,z,subs),3)-nanmean(BetaIpsiBins(:,z,subs),3)),'linestyle', '-', 'linewidth',3, 'color', colours_red{z+2});
    pl2(z)=plot(nanmean(Plot_RTBins(:,z,subs),3),nanmean(PreEvSlopeContra(:,z,subs)-PreEvSlopeIpsi(:,z,subs),3)*-1,'linestyle', '-', 'linewidth',3, 'color', colours_red{z+2});
